...
import top.hondaman.manux.framework.common.util.number.NumberUtils;
...
import top.hondaman.manux.framework.web.core.util.WebFrameworkUtils;
import top.hondaman.manux.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import top.hondaman.manux.module.bpm.controller.admin.task.vo.task.*;
import top.hondaman.manux.module.bpm.convert.task.BpmTaskConvert;
...
import top.hondaman.manux.module.bpm.enums.definition.*;
import top.hondaman.manux.module.bpm.enums.task.BpmCommentTypeEnum;
import top.hondaman.manux.module.bpm.enums.task.BpmReasonEnum;
import top.hondaman.manux.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import top.hondaman.manux.module.bpm.service.definition.BpmFormService;
import top.hondaman.manux.module.bpm.service.definition.BpmModelService;
...
import top.hondaman.manux.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
import static top.hondaman.manux.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 manux-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/manux-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/manux-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/manux-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
