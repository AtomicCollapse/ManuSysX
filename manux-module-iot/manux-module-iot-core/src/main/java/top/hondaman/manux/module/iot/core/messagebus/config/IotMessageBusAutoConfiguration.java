package top.hondaman.manux.module.iot.core.messagebus.config;

import top.hondaman.manux.framework.mq.redis.core.RedisMQTemplate;
import top.hondaman.manux.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import top.hondaman.manux.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import top.hondaman.manux.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import top.hondaman.manux.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import top.hondaman.manux.module.iot.core.messagebus.core.IotMessageBus;
import top.hondaman.manux.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import top.hondaman.manux.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import top.hondaman.manux.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "manux.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "manux.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    /**
     * 特殊：由于 ManuxRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see top.hondaman.manux.framework.mq.redis.config.ManuxRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "manux.iot.message-bus", name = "type", havingValue = "redis")
